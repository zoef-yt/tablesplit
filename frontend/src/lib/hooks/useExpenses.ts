...
export function useExpenses(groupId: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await apiHelpers.get<Expense[]>(`/expenses/group/${groupId}`);
...
export function useBalances(groupId: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await apiHelpers.get<Balance[]>(`/expenses/group/${groupId}/balances`);
      return response.data || [];
    },
    enabled: !!groupId,
  });
...
export function useSettlements(groupId: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await apiHelpers.get<Settlement[]>(`/expenses/group/${groupId}/settlement`);
      return response.data || [];
    },
    enabled: !!groupId,
  });
...
export function useRecordSettlement(groupId: string) {
  ...
  return useMutation({
    mutationFn: async (data: { from: string; to: string; amount: number }) => {
      const response = await apiHelpers.post(`/expenses/group/${groupId}/settle`, data);
      return response.data;
    },
